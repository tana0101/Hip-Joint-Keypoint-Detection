...
class HRNetBackbone(nn.Module):
    """
    純 HRNet backbone，forward(x) 回傳最後高解析度 branch feature: [B, C, H, W]
    不包含 heatmap / SimCC head。
    """
    def __init__(self, cfg):
        super().__init__()
        extra = cfg.MODEL.EXTRA
        self.inplanes = getattr(cfg.MODEL, "STEM_INPLANES", 64)

        # stem
        self.conv1 = nn.Conv2d(3, self.inplanes, 3, 2, 1, bias=False)
        self.bn1   = nn.BatchNorm2d(self.inplanes, momentum=BN_MOMENTUM)
        self.conv2 = nn.Conv2d(self.inplanes, self.inplanes, 3, 2, 1, bias=False)
        self.bn2   = nn.BatchNorm2d(self.inplanes, momentum=BN_MOMENTUM)
        self.relu  = nn.ReLU(inplace=True)
        ...
        # ⭐ 對外暴露最後高解析度 branch 的 channel 數
        self.backbone_out_channels = pre_stage_channels[0]

    # === 以下 helper 都是從你貼的 PoseHighResolutionNet 拿下來 ===
    ...
    def forward(self, x):
        # 和原來 PoseHighResolutionNet.forward_backbone 一樣
